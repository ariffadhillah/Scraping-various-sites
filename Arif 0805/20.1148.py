...
url = 'https://www.judicial.ky/judicial-administration/justices-of-appeal'
...
discription_ = find_section.find_all('div', {'class': ['panel-body toggle-content fusion-clearfix']})
title_ = find_section.find_all('div', {'class': ['fusion-text fusion-text-1', 'fusion-text fusion-text-2']})

sections = soup.find_all('div', class_='post-content')
titles_and_names_seen = set()

for section in sections:
    title_divs = section.find_all('div', {'class': ['fusion-text fusion-text-1', 'fusion-text fusion-text-2']})
    for title_div in title_divs:
        ...
        for name_anchor in name_anchors:
            ...
            if title_name_pair not in titles_and_names_seen:
                ...
                data_20_1148 = {
                    'Names' : name,
                    'Roles' :'Justices of Appeal',
                    'Titles' : current_title,
                    'PII': description.strip()
                }

                data.append(data_20_1148)
                print('Saving', data_20_1148['Names'],data_20_1148['Titles'])

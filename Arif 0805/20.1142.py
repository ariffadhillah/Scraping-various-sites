...
url = 'http://www.bvi.gov.vg/content/ministry-communication-and-works'
...
data = []
fields = ['Names' , 'Roles' , 'Office Telephone' , 'Office Email' , 'Office Fax' , 'Office Address' , 'Business Hours']

filename = '20.1142.csv'
...
for idx, (selector, name) in enumerate(zip(selectors, names_list)):
    ...
    if role_element:
        role = role_element.get_text().strip().replace('Mrs. Carleen Mercer-Scatliffe', '').replace('Ms. Haley Trott (Ag.)','').replace('Margaret Jones-Greene','')
        ...
        data.append(data_20_1142)
        print('Saving', data_20_1142['Names'])

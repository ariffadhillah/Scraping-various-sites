...
fields = ['Name' ,'Company Name','Phone Number', 'Email' , 'Address', 'Url Site', 'List Name']
filename = '90.1191.csv'
data = []

r = requests.get(baseurl, headers=headers)
soup = BeautifulSoup(r.content, 'lxml')

panelGroups = soup.find_all('div', class_='panel-group')[0]
...
for panelGroup in panelGroups:
    ...
    for cardItem in cardItem:
        ...
        try:
            siteURL = cardItem.find('span', class_='fas fa-globe')
            urlSite = siteURL.find_next_sibling('a')
            siteUrl = urlSite.get_text()
        except:
            siteURL = ''
            urlSite = ''
            siteUrl = ''
        ...
        data_90_1191 = {
            ...
            'Address' : addres,
            'Url Site' : siteUrl,
            'List Name': 'Securities Exchange'
        }

        data.append(data_90_1191)
        print('Saving', data_90_1191['Company Name'])
